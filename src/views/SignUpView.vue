...
const signUp = () => {
  ...
  axios.post('http://127.0.0.1:3005/api/users/sign_up' ,user.value)
    .then((res) => {
      console.log(res);
...
<template>
  <AuthPanel subtitle="註冊">
    <form action="" @submit.prevent="signUp">
      <div class="mt-4">
        <input type="text" id="name" name="name" placeholder="暱稱" class="w-full border-slate-900 border-2 px-4 py-3" required v-model="user.name">
      </div>
      <div class="mt-4">
      ...
      <div class="mt-8">
        <p class="text-sm text-red-400 text-center mt-4" v-if="errorMessage">{{ errorMessage }}</p>  
      </div>
      
      <Button text="註冊" buttonType="submit" :buttonDisabled="buttonDisabled" class="mt-4"/>
